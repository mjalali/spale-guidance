...
class SPARKEGuidedStableDiffusionXLPipeline(
    DiffusionPipeline,
    ...
    def get_image_from_latent(self, latents, noise_pred=None, t=None,update_latents_from_noise=False,
        ...
        if needs_upcasting:  # TODO commented because of memmory overflow
            self.upcast_vae()
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # **** CUSTOM GUIDANCE BLOCK (Adding Vendi and RKE guidance) ****
                # Check if custom guidance (e.g., based on vscore_clip) is enabled.
                # Here, criteria_guidance_scale, clip_for_guidance, and guidance_freq come from your custom settings.
                # if (
                #     self.do_classifier_free_guidance
                #     and criteria_guidance_scale != 0
                #     and clip_for_guidance is not None
                # ):
                # raise NotImplementedError("Custom guidance is not implemented yet.")
                assert rke_guided_sampler is not None
                clip_for_guidance = kwargs['clip_for_guidance']
                guidance_freq = kwargs['guidance_freq']
                criteria = kwargs['criteria']
                criteria_guidance_scale = kwargs['criteria_guidance_scale']
                F_M = kwargs['F_M']
                F_T = kwargs['F_T']
                F_M_real = kwargs['F_M_real']
                F_T_real = kwargs['F_T_real']
                beta = kwargs['beta']
                logger = kwargs['logger_']
                regularize = kwargs['regularize']
                regularize_weight = kwargs['regularize_weight']
                if criteria_guidance_scale != 0 and clip_for_guidance:  # TODO check if it's required
                    # Optionally use the conditional part of the prompt embeddings for guidance
                    ...
                    if i % guidance_freq == 0 and criteria == 'vscore_clip':
                        ...
                        if torch.isnan(grads).any() or torch.isinf(grads).any():
                            logger.info("Skipping gradient update due to NaN or Inf in grads.")
                        else:
                            # Update latents with the computed guidance gradient
                            latents = latents + grads
                # **** END CUSTOM GUIDANCE BLOCK ****
