...
class SPARKEGuidedDiTPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        # Parameters for custom guidance
        ...
        **kwargs,
        ...
        for i, t in enumerate(self.progress_bar(self.scheduler.timesteps)):
            ...
            if rke_guided_sampler is not None:
                clip_for_guidance = kwargs.get('clip_for_guidance')
                guidance_freq = kwargs.get('guidance_freq', 1)
                criteria = kwargs.get('criteria')
                criteria_guidance_scale = kwargs.get('criteria_guidance_scale', 0)
                F_M = kwargs.get('F_M')
                F_T = kwargs.get('F_T')
                F_M_real = kwargs.get('F_M_real')
                F_T_real = kwargs.get('F_T_real')
                beta = kwargs.get('beta')
                logger = kwargs.get('logger_')
                regularize = kwargs.get('regularize')
                regularize_weight = kwargs.get('regularize_weight')
                ...
                if criteria_guidance_scale != 0 and clip_for_guidance:
                    if i % guidance_freq == 0 and criteria == 'vscore_clip':
                        grads, F_M, F_T = rke_guided_sampler.cond_fn(
                            ...
                            prompt=str(class_labels_input[0].item()),  # TODO, check if this is actually works for clip guidance
                        ...
                        if torch.isnan(grads).any() or torch.isinf(grads).any():
                            ...
                            latent_model_input = latent_model_input + grads
        ...
        if return_kernels and rke_guided_sampler is not None:
            return (samples, F_M, F_T)
