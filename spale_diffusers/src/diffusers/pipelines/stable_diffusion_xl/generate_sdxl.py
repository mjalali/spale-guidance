from stable_diffusion_xl_vendi_pipeline import RKEGuidedStableDiffusionXLPipeline
...
vendi_dir = '/home/student/Documents/evaluation/rke_guidance/Contextualized-Vendi-Score-Guidance'
import sys
sys.path.insert(0, vendi_dir)


from utils import precompute_F_M_F_T_real, image_to_pil, get_F_M
...
def generate(dfs_n_csv_path, clip_for_guidance, pre_process_clip, clip_model, feature_extractor, num_real_samples,
    ...
    criteria_pipe = RKEGuidedStableDiffusionXLPipeline.from_pretrained(
    ...
    for index, row in df.iterrows():
        ...
        if guidance_type == 'vscore_clip' and initial_bank_per_object is True and obj_ref != obj:
            ...
            if F_M_real_all is not None:
                F_M_real = F_M_real_all[obj.lower()][0]
                F_T_real = F_M_real_all[obj.lower()][1]
...
def main():
    ...
    parser.add_argument('--root_dir', type=str, default=f'{vendi_dir}/generated_datasets/toy_dataset',
